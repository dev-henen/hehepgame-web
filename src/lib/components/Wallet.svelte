...
<div class="p-2">
  ...
  <!-- Balance Section -->
  <div class="bg-[#131214] rounded-2xl shadow-lg p-4 mb-6">
    <h3 class="text-lg font-semibold">
      <i class="fa-solid fa-money-bill-wave mr-1"></i>Current Balance
    </h3>
  ...
  <div class="flex mb-4 border-b border-gray-700">
    <button
      class={`px-4 py-2 font-semibold flex items-center gap-2 ${activeTab === "fund" ? "border-b-2 border-green-500 text-green-500" : "text-gray-500"}`}
      on:click={() => (activeTab = "fund")}
    >
      <i class="fa-solid fa-circle-plus"></i> Fund Wallet
    </button>
    <button
      class={`px-4 py-2 font-semibold flex items-center gap-2 ml-4 ${activeTab === "withdraw" ? "border-b-2 border-green-500 text-green-500" : "text-gray-500"}`}
  ...
  {#if activeTab === "fund"}
    <div class="mb-6">
      <h4 class="text-lg font-semibold mb-2">
        <i class="fa-solid fa-coins mr-1"></i>Add Funds
      </h4>
      <input
        type="number"
  ...
  {:else if activeTab === "withdraw"}
    <div class="mb-6">
      <h4 class="text-lg font-semibold mb-2">
        <i class="fa-solid fa-hand-holding-dollar mr-1"></i>Withdraw Funds
      </h4>
      <input
        type="number"
        placeholder="Enter amount"
  ...
  <div>
    <h4 class="text-lg font-semibold mb-2">
      <i class="fa-solid fa-clock-rotate-left mr-1"></i>Transaction History
    </h4>
    {#if history.length === 0}
    ...
    {:else}
      <ul class="space-y-2">
        {#each history as item}
          <li
            class="bg-[#131214] rounded-2xl shadow-lg p-3 flex justify-between items-center"
          >
            <div class="flex items-center gap-3">
              <i
                class={`text-xl ${item.type === "Deposit" ? "fa-solid fa-arrow-down text-green-600" : "fa-solid fa-arrow-up text-red-500"}`}
              ></i>
              <div>
            ...
            <span
              class={`font-bold ${item.type === "Withdrawal" ? "text-red-500" : "text-green-600"}`}
            >
